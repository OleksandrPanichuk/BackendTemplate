...
export class MailerService implements OnModuleInit {
  ...
  constructor(private readonly config: ConfigService) {
    this.defaultFrom = this.config.get<string>(Env.MAIL_FROM)!;

    this.transporter = createTransport({
      host: config.get(Env.MAIL_HOST),
      port: parseInt(config.get(Env.MAIL_PORT)!),
      secure: true,
      auth: {
  ...
  private renderTemplate(
    ...
    const templateVariables = {
      appName: this.config.get<string>(Env.APP_NAME) || 'Our App',
      currentYear: new Date().getFullYear(),
      supportEmail:
        this.config.get<string>(Env.SUPPORT_EMAIL) || 'support@example.com',
      ...variables,
    };
